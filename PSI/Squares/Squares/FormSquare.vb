...
Public Class FormSquares
    ...
    Dim _originalX As Integer
    Dim _originalY As Integer
    ...
    Private Sub AddSquareBtn_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles AddSquareBtn.Click
        ...
        If Integer.TryParse(WidthTextBox.Text, intWidth) Then
            If Integer.TryParse(LengthTextBox.Text, intLength) Then
                If Integer.TryParse(TextBoxSpeed.Text, intSpeed) Then
                        ...
                        If Integer.TryParse(TextBoxPositionStartY.Text, intYstart) Then
                            If Integer.TryParse(TextBoxEndPositionX.Text, intXend) Then
                                If Integer.TryParse(TextBoxEndPositionY.Text, intYend) Then
                                    ...
                                    mySquare.Speed = System.Convert.ToInt32(TextBoxSpeed.Text)
                                    ...
                                    mySquare.OriginalX = mySquare.StartXposition
                                    mySquare.OriginalY = mySquare.StartYposition
                                    '---------------------------------------------------------------
                    ...
                    End If
                Else
                    MessageBox.Show("Please enter an Integer into the Speed text box.")
                    TextBoxSpeed.Clear()
                    TextBoxSpeed.Focus()
                End If
    ...
    Public Sub MoveSquares(ByVal _squares As SquareCollection)
        ...
        For Each tempSquare As Square In Me._squares
            tempSquare.Move()
            '-----------------------------------------------------------------------------------------------------------------------------
            ' Drawing the square for each square in collection
            Me.DrawSquarePictureBox.Refresh()
            Brush.Color = tempSquare.Color                              ' The brush to draw with
            drawingSurface.FillRectangle(Brush, tempSquare.StartXposition, _
                                         tempSquare.StartYposition, tempSquare.Width, _
                                         tempSquare.Height)         ' Draw the rectangle on the graphics (x, y is the starting location) 
            drawingSurface.DrawRectangle(LinePen, tempSquare.StartXposition, _
                                         tempSquare.StartYposition, tempSquare.Width, _
                                         tempSquare.Height)       ' Draw an outline around the rectangle (x, y is the starting location)
            Me.DrawSquarePictureBox.Image = picture
            '-----------------------------------------------------------------------------------------------------------------------------
        Next
    End Sub
    '---------------------------------------------------
    ' Reset Squares Button  (Previously Draw Button)
    '---------------------------------------------------
    Private Sub DrawSquareButton_Click(ByVal sender As System.Object, ByVal e As System.EventArgs) Handles DrawSquareButton.Click
        For Each mySquare As Square In Me._squares
            mySquare.ResetStartPosition(Me._originalX, Me._originalY)
            Me.DrawSquarePictureBox.Refresh()
        Next

        '----------------------------------------------------------------------------
        ' -- Used before the animation was added -- 
